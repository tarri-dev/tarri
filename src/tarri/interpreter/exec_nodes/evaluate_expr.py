...
def evaluate_expr(self, node):
    ...
    if isinstance(node, Tree):
        ...
        if node.data == "grouped_expr":
            ...
            modul = self.evaluate_expr(node.children[0])
            aksi_node = node.children[1]

            aksi = aksi_node.value if isinstance(aksi_node, Token) else str(aksi_node)
            ...
            return self.call_function(modul, [aksi] + args)
            ...
            def replacer(match):
                ...
                if "[" in expr_code:
                    ...
                    if ".." in rest:  # slice
                        ...
                        start = Token("NUMBER", start_str) if start_str.isdigit() else Token("VAR_NAME", start_str)
                        end   = Token("NUMBER", end_str) if end_str.isdigit() else Token("VAR_NAME", end_str)
        ...
        # OR (||)
            ...
            # SLICE EXPR: selalu kembalikan sublist
                ...
                start_node = idx_node.children[0] if len(idx_node.children) > 0 else None
                ...
                try:
                    ...
                    self.error(f"Slice gagal: start={start}, end={end}, error={e}")
                    return None
